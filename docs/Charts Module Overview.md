# Charts Module â€“ Functional Overview

Dieses Dokument beschreibt das komplette SVG-Chart-Modul (Blutdruck- und KÃ¶rperdiagramm) im Gesundheits-Logger. Es dient als Nachschlagewerk fÃ¼r Entwickler, damit klar ist, welche Dateien zusammenwirken, wie Daten flieÃŸen und welche UX/Diagnose-Eigenschaften implementiert sind.

---

## 1. Zielsetzung

Die Charts visualisieren Tageswerte (Blutdruck morgen/abend, KÃ¶rpergewicht/Bauchumfang, Muskel-/Fettanteile) inklusive KPI-Leiste, interaktiven Tooltips und Trendpilot-Layern. Sie sollen:

- Messwerte eines Zeitraums (Standard: â€žHeute â€“ 90 Tageâ€œ) aufbereiten.
...
- Accessibility (Keyboard, ARIA) gewÃ¤hrleisten.
- Besondere ZustÃ¤nde hervorheben (Pulse-Link, Trendpilot-BÃ¤nder, MAP/Pulsdruck-Indikatoren).
...
2. Falls Nutzer eingeloggt: `fetchDailyOverview(from, to)` (Supabase) â€“ liefert strukturierte Tagesobjekte (`morning`, `evening`, `weight`, `notes`).
3. Transformiert in flache EintrÃ¤ge (`context` + `ts`) fÃ¼r Morgen/Abend + Tagessummary.
...
2. Nutzt `series`-Definitionen:
   - BP: `Sys/Dia` (Morgen/Abend) â†’ Linien + Punkte, Pulse-Pressure und MAP berechnet.
   - Body: Gewicht/Bauchumfang â†’ Linien; Muskel/Fett in `barSeries`.
...
4. Rendering:
   ...
   - Pulse-Link: Bei BP-Hover wird das Messpaar (Sys/Dia) Ã¼ber `<line>` verbunden.
   - Zielbereiche (â€žGoal Bandsâ€œ) und Trendpilot-BÃ¤nder (`<rect class="trendpilot-band ...">`).
   - Grid/Achsen + Schwellenlinien (130/90 mmHg).
   - Legende: gilt fÃ¼r Linien/Bars + Trendpilot-Swatches.
...
5. Pulse-Link wird nur gezeichnet, wenn es fÃ¼r beide Messungen (Morning/Evening) valide Koordinaten gibt.
...
- `chartPanel.applyChartAnimations`: setzt stroke-dashoffset/opacity/scale fÃ¼r Linien/Punkte/Bars.
...
1. **BP-Chart**
   - ESC-2018 Tooltip-Hintergrund (optimal â†’ Grad III).
   ...
   - Trendpilot-BÃ¤nder hinter den Linien.

2. **Body-Chart**
   - Gewicht + Bauchumfang Linien, Muskeln/Fett Bars (relativ zu 75â€¯kg Baseline).
...
4. **Accessibility**
   ...
   - ARIA-Live Region fÃ¼r Screenreader.
...
- Konsolenwarnungen, falls `CSS.escape`/`document` nicht verfÃ¼gbar (Fallbacks).
- Animationsflag kann deaktiviert sein â€“ Chart prÃ¼ft Flag + `prefers-reduced-motion`.

---

## 6. Speicher-/Flow-Zusammenspiel

1. **Capture Tab** sammelt Tagesdaten â†’ `fetchDailyOverview`.
2. **Doctor Tab** bestimmt Range + ruft Chart-Panel (Button) oder `setTab('doctor')`.
3. **Trendpilot** sendet BÃ¤nder per `loadTrendpilotBands`.
4. **Supabase** (via `supabase/index.js`) liefert API fÃ¼r `fetchDailyOverview` + `fetchSystemCommentsRange`.
...
- ZusÃ¤tzliche Metriken (z.â€¯B. Pulsdruck-Linie, MAP-Bars).
- Export-Funktion (SVG/PNG).
- Konfigurierbare Range-Selector (z.â€¯B. 30/60/180 Tage).
- Theme/Color-Adjustments mittels CSS Custom Properties.

---

Bei Ã„nderungen am Chart (z.â€¯B. neue Serien, zusÃ¤tzliche Tooltips) sollte diese Datei aktualisiert werden, damit zukÃ¼nftige Arbeiten sich auf dieselbe Wissensbasis stÃ¼tzen kÃ¶nnen.
