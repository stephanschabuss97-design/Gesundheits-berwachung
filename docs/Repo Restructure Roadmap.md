...
1. ✅ **Styles verschoben:**
   - Kern-CSS liegt jetzt unter pp/styles/ (inkl. ui.css).
   - Chart-Styles unter pp/modules/charts/chart.css.
   - Neuer Composer pp/app.css importiert alle Teilstyles.
   - Lessons learned: Deploy-Pipeline vorbereiten, Chart-Snapshots testen bevor Pfade umgestellt werden.
2. ✅ **Core JS verschieben:**
   - ssets/js/config.js, utils.js, diag.js, capture/globals.js → pp/core/....
3. ✅ **Supabase Barrel angleichen:**
   - ssets/js/supabase/index.js + Submodule (core/, uth/, pi/, ealtime/) nach pp/supabase/ kopiert.
4. 🔲 **Smoke-Tests & Pages-Check:** Capture, Doctor, Chart, Trendpilot mit den neuen Dateien (lokal + Pages/Disable cache) testen; MODULE-/SUBMODULE-Kommentare prüfen.
5. 🔲 **Referenzen umstellen:** <link>/<script> in index.html (und ggf. oot-auth.js) schrittweise auf pp/... ändern; ssets/... nur noch als Fallback.
6. 🔲 **Altbestände abbauen:** Wenn pp/... stabil läuft, doppelte ssets/css/ssets/js-Dateien entfernen oder als Build-Artefakt kennzeichnen.

### Erfolgskriterien
- Alle <script src="..."> und <link rel="stylesheet" href="..."> zeigen dauerhaft auf pp/...; GitHub-Pages-Check ohne 404.
...
- Lessons learned: Vorher einen Chart-Snapshot testen (Linien + Punkte) und sicherstellen, dass pp/-Dateien wirklich deployed werden (Cache-Buster + Pages). Erst dann Pfade umstellen.
- Neue Module zuerst in pp/... hinzufügen, parallel testen/deployen und erst nach erfolgreicher Verifikation die alten ssets/...-Quellen entfernen.
...
1. ðŸ”² **Appointments/Training Re-Enable:** Verzeichnis anlegen, Placeholder-Dateien + TODO-Markierungen setzen.  
2. ðŸ”² **Assistant (KI):** Unter `app/modules/assistant/` API + Prompt-Struktur vorbereiten; Keys via Supabase Functions, damit keine Secrets im Client landen.
3. ðŸ”² **Diagnostics:** `app/diagnostics/` nur im Dev-Build laden (Feature-Flag in `app/core/config.js`).
4. ðŸ”² **PWA/TWA-Feinschliff:**  
...
1. ðŸ”² **Assets-Ordner aufrÃ¤umen:** Entferne alte `assets/`-Reste, sobald alle Pfade angepasst sind.  
2. ðŸ”² **README & Onboarding:** Neue Struktur erklÃ¤ren, Setup-Schritte aktualisieren.  
3. ðŸ”² **Automatisierte Tests:** falls vorhanden (z.â€¯B. Playwright), Pfade anpassen.  
4. ðŸ”² **Release Checklist:**  
   - `CHANGELOG.md` zusammenfassen.  
   - QA-Run (Unlock, Capture, Doctor, Trendpilot, Chart, Offline).  
   - Tag/Release (z.â€¯B. `vNext-restructure`).
- Im Cleanup weiterhin das â€žNeu zuerst, dann Entfernenâ€œ-Prinzip anwenden (z.â€¯B. `assets/` erst lÃ¶schen, wenn GitHub Pages definitiv nur noch `app/` benÃ¶tigt).
