...
const mkBars = () => {
  ...
  activeBars.forEach((seriesItem, idx) => {
    ...
    seriesItem.values.forEach((rawVal, i) => {
      ...
      out += `<rect class="chart-bar" x="${rectX.toFixed(1)}" y="${rectY.toFixed(1)}" width="${barWidth.toFixed(1)}" height="${rectH.toFixed(1)}" fill="${seriesItem.color}" opacity="0.35"${seriesItem.key ? ` data-series="${esc(seriesItem.key)}"` : ""}></rect>`;
    });
  });
  return out;
};
