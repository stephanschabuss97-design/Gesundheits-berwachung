...
let sbClient = null;
let __cachedHeaders = null;
let __cachedHeadersAt = 0;
let __intakeRpcDisabled = false;
...
async function fetchWithAuth(makeRequest, { tag = '', retry401 = true, maxAttempts = 2 } = {}) {
  ...
  const loadHeaders = async (forceRefresh = false) => {
    ...
    if (!forceRefresh && __cachedHeaders && __cachedHeadersAt && (Date.now() - __cachedHeadersAt) < 5 * 60 * 1000) {
      diag.add?.('[headers] cache hit');
      return __cachedHeaders;
    }
...
function watchAuthState(){
  sbClient.auth.onAuthStateChange(async (event, session)=>{
    ...
    if (logged) {
      ...
      if (newUid) {
        __lastUserId = newUid;
        diag.add?.(`[auth] session uid=${newUid}`);
      }
...
// SUBMODULE: resetCapturePanels @internal - kombiniert Panel-Resets (BP/Body/Flags) und setzt Kontext zurÃ¼ck
...
// SUBMODULE: addCapturePanelKeys @internal - registriert TastaturkÃ¼rzel fuer Save-/Reset-Flows
...
async function loadIntakeToday({ user_id, dayIso }){
  ...
  diag.add?.(`[capture] loadIntakeToday start uid=${user_id} day=${dayIso||''}`);
...
const supa = await ensureSupabaseClient();
if(!supa) {
  if ((__authState === 'auth' || __authState === 'unknown') && __lastUserId) {
    diag.add?.(`[auth] getUserId fallback (no client) ${__lastUserId}`);
    return __lastUserId;
  }
  return null;
}
...
try {
  ...
  if (timedOut) {
    ...
    if ((__authState === 'auth' || __authState === 'unknown') && __lastUserId) {
      diag.add?.(`[auth] getUserId fallback (timeout) ${__lastUserId}`);
      return __lastUserId;
    }
  }
  throw err;
...
if (uid) {
  __lastUserId = uid;
  diag.add?.(`[auth] getUserId done ${uid}`);
  return uid;
}
if ((__authState === 'auth' || __authState === 'unknown') && __lastUserId) {
  diag.add?.(`[auth] getUserId fallback (no uid) ${__lastUserId}`);
  return __lastUserId;
}
...
if ((__authState === 'auth' || __authState === 'unknown') && __lastUserId) {
  diag.add?.(`[auth] getUserId fallback (error) ${__lastUserId}`);
  return __lastUserId;
}
return null;
}
